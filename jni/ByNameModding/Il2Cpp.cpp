...
namespace {
    ...
    const void *(*il2cpp_image_get_class)(const void *image, size_t index);
    const char *(*il2cpp_class_get_name)(const void *klass);
    const char *(*il2cpp_class_get_namespace)(const void *klass);
...
void Il2CppAttach(const char *name) {
    // [修改 2] dlopen -> xdl_open
    // 使用 XDL_DEFAULT 模式，xDL 会自动处理命名空间限制
    void *handle = xdl_open(name, XDL_DEFAULT);
    
    if (!handle) {
        IL2CPP_LOGE("Failed to load library: %s", name);
        return;
    }

    // [修改 3] dlsym -> xdl_sym
    // 注意：xdl_sym 需要第三个参数 (size_t* symbol_size)，不需要时传 nullptr
    ...
    il2cpp_image_get_class_count = (size_t (*)(const void *))xdl_sym(handle, "il2cpp_image_get_class_count", nullptr);
    il2cpp_image_get_class = (const void *(*)(const void *, size_t))xdl_sym(handle, "il2cpp_image_get_class", nullptr);
    il2cpp_class_get_name = (const char *(*)(const void *))xdl_sym(handle, "il2cpp_class_get_name", nullptr);
    il2cpp_class_get_namespace = (const char *(*)(const void *))xdl_sym(handle, "il2cpp_class_get_namespace", nullptr);
...
void *Il2CppFindClass(const char *image, const char *namespaze, const char *clazz, const char *verifierMethodName) {
    ...
    for (size_t i = 0; i < classCount; ++i) {
        const void* current_klass = il2cpp_image_get_class(img, i);
        if (!current_klass) continue;

        // 比较类名和命名空间
        const char* className = il2cpp_class_get_name(current_klass);
        const char* classNamespace = il2cpp_class_get_namespace(current_klass);

        if (strcmp(className, clazz) == 0 && strcmp(classNamespace, namespaze) == 0) {
            // 找到了一个同名类！现在进行验证。
            // 尝试从这个类中获取“验证方法”
            if (verifierMethodName) {
                if (il2cpp_class_get_method_from_name((void*)current_klass, verifierMethodName, 0)) {
                    // 成功找到了验证方法！这说明是我们想要的那个类！
                    
                    return (void*)current_klass;
                }
            } else {
                // 如果没有提供验证方法，就返回第一个找到的
                return (void*)current_klass;
            }
        }
    }
...
void *Il2CppGetMethodOffsetWithVerifier(const char *image, const char *namespaze, const char *clazz, const char *name, int argsCount) {
        // 使用新的查找逻辑，将方法名本身作为验证器
        void *klass = Il2CppFindClass(image, namespaze, clazz, name);

        if(!klass) {
            // Il2CppFindClass 内部已经显示了Toast
            return 0;
        }
